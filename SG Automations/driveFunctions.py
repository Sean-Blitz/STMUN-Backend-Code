...
def create_drive_folder(service, name, mime_type, parent_id=None):
    """
...
def copy_drive_file(service, file_id, destination_folder_id = None, new_name=None):
    """
...
def copy_drive_file_with_number(service, original_file_id, destination_folder_id, new_name_template, sName):
    """
...
def find_subfolder_id(service, parent_folder_id, search_string):
    """
...
def move_drive_folder(service, folder_id, new_parent_folder_id):
    """
    ...
    # 2. Move the folder
    moved_folder = service.files().update(
        fileId=folder_id,
...
def share_doc_with_user(drive_service, document_id, email, role="writer"):
    """
...
def get_subfolders_as_dict(drive_service, PARENT_FOLDER_ID):
    """
...
def find_sheet_id_by_name_contains(drive_service, folder_id, name_contains):
    """
...
def list_google_sheet_ids(folder_id: str, service) -> list:
    """
